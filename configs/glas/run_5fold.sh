...
for fold in 0 1 2 3 4; do
  run_name="glas_5fold_fold${fold}_vit_query_mul_scale_fusion_lr_1em3"
  fold_root="datasets/Glas_5fold/fold${fold}"

  echo "============================================================"
  echo "GlaS 5-fold | fold=${fold} | TRAIN"
  echo "============================================================"
  ...
  status=$?
  if [ $status -ne 0 ]; then
    echo "[WARN] Training failed for GlaS fold${fold} with exit code ${status}. Continuing."
    continue
  fi
done
...
python scripts/summarize_5fold_best.py \
  runs \
  "glas_5fold_fold" \
  "runs/glas_5fold_vit_query_mul_scale_fusion_lr_1em3_summary.txt"
