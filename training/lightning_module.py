...
class LightningModule(lightning.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step == 0:
            self.print(f"[TRAIN] imgs dtype={imgs.dtype}, min={imgs.min().item():.4f}, max={imgs.max().item():.4f}")
            assert imgs.max() <= 1.5, "Your imgs are not in [0,1]. Check transforms / normalization."
    ...
    def test_step(self, batch, batch_idx):
        # batch: dict from your case-level dataset
        case = batch["case_name"]
        img_vol = batch["image"]  # [D,H,W] float32
        gt_vol  = batch["label"]  # [D,H,W] int64

        if img_vol.dim() == 4:  # sometimes [1,D,H,W]
            img_vol = img_vol[0]
        if gt_vol.dim() == 4:
            gt_vol = gt_vol[0]

        img_vol_np = img_vol.cpu().numpy()                    # [D,H,W]
        gt_vol_np  = gt_vol.cpu().numpy().astype(np.int32)    # [D,H,W]
        D, H, W = img_vol_np.shape

        patch_h, patch_w = self.img_size  # e.g. (384,384)
        ...
        with torch.no_grad():
            for z in range(D):
                slc = img_vol_np[z]  # [H,W]
                x, y = slc.shape

                # (A) resize image -> patch size (same as TransUNet)
                ...
                mask_logits  = mask_logits_per_block[-1][0]   # [Q,h,w] (h,w ~ 96)
                class_logits = class_logits_per_block[-1][0]  # [Q,C+1]

                # per-pixel logits at low-res: [C,h,w]
                per_pix = self.to_per_pixel_logits_semantic(
                    mask_logits.unsqueeze(0),
                    class_logits.unsqueeze(0),
                )[0]

                # (B) upsample logits -> patch size FIRST, then argmax
                ...
                pred_patch = torch.argmax(per_pix, dim=0).to(torch.int32).cpu().numpy()  # [patch_h,patch_w]

                # (C) resize pred back -> original slice size (nearest, same as TransUNet)
                if (x != patch_h) or (y != patch_w):
        ...
        # ---- metrics (TransUNet style: class 1..C-1) ----
        metric_list = []
