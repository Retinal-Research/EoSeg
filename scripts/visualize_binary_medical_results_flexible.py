...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize binary medical segmentation predictions across multiple dataset layouts."
    )
    ...
    parser.add_argument("--test-mask-dir", type=Path, default=None)
    parser.add_argument("--test-split-file", type=Path, default=None)
...
def collect_pairs_from_dirs(
    ...
    normalized_image_suffix = image_suffix.lower() if image_suffix else None
    pairs = []
    for image_path in sorted(image_dir.iterdir()):
        if not image_path.is_file():
            continue
        if image_path.name.startswith(".") or image_path.name.startswith("._"):
            continue
        if normalized_image_suffix and image_path.suffix.lower() != normalized_image_suffix:
            continue
...
def resolve_split_file(dataset_dir: Path, split_name_or_path: str | Path | None):
    if split_name_or_path is None:
        return None

    split_path = Path(split_name_or_path)
    if split_path.is_file():
        return split_path

    candidate = dataset_dir / f"{split_path}.txt"
    if candidate.is_file():
        return candidate

    raise FileNotFoundError(f"Split file not found: {split_path} or {candidate}")


def resolve_file(folder: Path, stem_or_name: str):
    base = folder / stem_or_name
    if base.is_file():
        return base

    for ext in (".jpg", ".JPG", ".png", ".PNG", ".jpeg", ".JPEG", ".bmp", ".BMP", ".tif", ".tiff"):
...
def collect_pairs_from_split(dataset_dir: Path, split_file: Path):
    ...
    pairs = []
    for raw_name in split_file.read_text(encoding="utf-8").splitlines():
        name = raw_name.strip()
        if not name:
            continue
        image_path = resolve_file(image_dir, name)
        mask_path = resolve_file(mask_dir, name)
        pairs.append((image_path, mask_path))
    return pairs


def collect_pairs_from_dirs_and_split(
    image_dir: Path,
    mask_dir: Path,
    split_file: Path,
):
...
def infer_pairs_from_config(config: dict, args):
    data_init = config.get("data", {}).get("init_args", {})

    if args.test_dir is not None:
        return collect_pairs(args.test_dir)

    if args.test_image_dir is not None and args.test_mask_dir is not None:
        if args.test_split_file is not None:
            return collect_pairs_from_dirs_and_split(
                args.test_image_dir,
                args.test_mask_dir,
                Path(args.test_split_file),
            )
        data_init = config.get("data", {}).get("init_args", {})
        return collect_pairs_from_dirs(
            args.test_image_dir,
            args.test_mask_dir,
            image_suffix=args.image_suffix or data_init.get("image_suffix"),
            mask_suffix=args.mask_suffix or data_init.get("mask_suffix", ".png"),
        )

    if args.test_image_dir is not None or args.test_mask_dir is not None:
        raise ValueError("Both --test-image-dir and --test-mask-dir must be provided together.")

    dataset_dir = data_init.get("dataset_dir")
    if dataset_dir:
        dataset_dir = Path(dataset_dir)
        split_file = resolve_split_file(dataset_dir, args.test_split_file or data_init.get("test_split") or data_init.get("val_split"))
        return collect_pairs_from_split(dataset_dir, split_file)

    test_image_dir = data_init.get("test_image_dir") or data_init.get("val_image_dir")
    test_mask_dir = data_init.get("test_mask_dir") or data_init.get("val_mask_dir")
    if test_image_dir and test_mask_dir:
        return collect_pairs_from_dirs(
            Path(test_image_dir),
            Path(test_mask_dir),
            image_suffix=args.image_suffix or data_init.get("image_suffix"),
    ...
    test_dir = data_init.get("test_dir") or data_init.get("val_dir")
    if test_dir:
        return collect_pairs(Path(test_dir))

    raise ValueError(
        "Could not infer test image/mask pairs from CLI arguments or config. "
        "Provide --test-dir, --test-image-dir/--test-mask-dir, or a config with test paths."
    )
...
def infer_binary_mask(model, image_tensor: torch.Tensor, output_size: tuple[int, int], threshold: float, device: torch.device):
    with torch.no_grad():
        ...
        if hasattr(model, "_foreground_logits_and_probs"):
            _, probs = model._foreground_logits_and_probs(inputs)
        else:
            outputs = model(inputs)
            if not (isinstance(outputs, tuple) and len(outputs) == 2):
                raise TypeError("Unsupported model output format for binary inference.")

            mask_logits_per_block, class_logits_per_block = outputs
            if not (isinstance(mask_logits_per_block, list) and isinstance(class_logits_per_block, list)):
                raise TypeError("Expected query-based mask/class output lists for binary inference.")
...
def main():
    ...
    pairs = infer_pairs_from_config(config, args)
